...
class OrmQueryExecutor {
  ...
  async execute(options = {}) {
    ...
    for (let serializerGroup of this.context.serializerGroups) {
      await Promise.all(serializerGroup.map(async serializer => {
        ...
        this.context.result = await serializeFunc(this.context);
      }));
    }
  ...
  /*async eagerLoad() {
    if (!this.context.eagerLoads.length) {
      return;
    }

    const adapter = this.context.adapter;
    await Promise.all(this.context.eagerLoads.map(async eagerLoad => {
      await Promise.resolve(adapter.eagerLoad(this.context, eagerLoad.relations, eagerLoad.options));
    }));
  }*/
}

...
class OrmQueryExecutor {
  ...
  async execute(options = {}) {
    ...
    this.context.executeQuery = this.executeQuery.bind(this);
    this.context.executeCountQuery = this.executeCountQuery.bind(this);
    ...
    await this.emit('end', async () => {
      this.context.result = await this.executeQuery(this.context.query);
    });
  ...
  async executeQuery(query) {
    return await this.emit('query', () => Promise.resolve(this.context.adapter.executeQuery(query, this.context)));
  }

  async executeCountQuery(query) {
    return await this.emit('query', () => Promise.resolve(this.context.adapter.executeCountQuery(query, this.context)));
  }

...
exports.applyJoinDefinition = function(context, joinDefinition) {
  if (joinDefinition.joinType && !_.includes(BOOKSHELF_JOIN_TYPES, joinDefinition.joinType)) {
    throw new Error(`Join type "${joinDefinition.joinType}" is not supported by Bookshelf`);
  }
  ...
  return context.query.query(qb => qb[joinType](joinTable, joinDefinition.column, joinDefinition.joinColumn));
};
...
exports.getJoinDefinitions = function(model, relation, options = {}) {
  ...
  if (typeof(relationFunc) !== 'function') {
    throw new Error(`Model ${model} has no relation "${relation}"`);
  }

  const relatedData = relationFunc.call(model.prototype).relatedData;
...
exports.getQueryIdentifier = function(context) {
  return context.query.query().toString();
};

...
class OrmQuerySortingHelper {
  ...
  async execute(context) {
    ...
    querySorts = _.filter(querySorts, sort => this.definedSorts[sort.name]);

    for (let sort of querySorts) {
      context.query = OrmQueryWrapper.unwrap(await Promise.resolve(this.definedSorts[sort.name](context.query, sort.direction, context)));
    }
...
module.exports = (...args) => new OrmQuerySortingHelper(...args);

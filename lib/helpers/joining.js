...
class OrmQueryJoiningHelper {
  ...
  addRequiredJoins(context) {
    ...
    const joinsToApply = _.values(_.pick(this.possibleJoins, sortedJoins));
  ...
  getSortedJoins(requiredJoins) {
    ...
    _.pull(remainingJoins, ...baseJoins);
    ...
    while (remainingJoins.length) {
      ...
      const dependentJoins = remainingJoins.filter(join => _.intersection(this.possibleJoins[join].requiredJoins, sortedJoins).length);
      sortedJoins.push(...dependentJoins);
      _.pull(remainingJoins, ...dependentJoins);
